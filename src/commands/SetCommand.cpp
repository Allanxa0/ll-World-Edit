...
ll::coro::CoroTask<void> executeSetTask(std::string xuid, BlockPos p1, BlockPos p2, DimensionType dim, const Block* newBlock) {
    ...
    if (count > WorldEditMod::getInstance().getConfig().maxBlocksPerOperation) {
        player->sendMessage("§cSet exceeds maximum block limit.");
        co_return;
    }
    ...
    for (const auto& [chunkPos, blocks] : chunkBatches) {
        ...
        for (const auto& targetPos : blocks) {
            ...
            if (oldBlock == *newBlock && !region.getBlockEntity(targetPos)) {
                continue;
            }
            ...
            region.setBlock(targetPos, *newBlock, 3, nullptr, context);
            replacedCount++;
    ...
    if (auto currentP = ll::service::getLevel()->getPlayerByXuid(xuid)) {
        ...
        currentP->sendMessage("§aSet " + std::to_string(replacedCount) + " blocks.");
    }
    co_return;
}
...
void registerSetCommand() {
    ...
    setCmd.overload<SetParams>()
        ...
        .execute([](CommandOrigin const& origin, CommandOutput& output, SetParams const& params) {
            ...
            auto blockOpt = Block::tryGetFromRegistry(HashedString(params.blockName));
            if (!blockOpt) {
                output.error("Invalid block name.");
                return;
            }
            const Block* newBlock = &(*blockOpt);

            executeSetTask(player->getXuid(), session.pos1.value(), session.pos2.value(), player->getDimensionId(), newBlock)
